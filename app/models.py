...
class Mission(TimestampMixin, db.Model):
    ...
    # regional_actions_summary = db.Column(db.Text)
...
class MissionPrisonReport(TimestampMixin, db.Model):
    ...
    visited_entity = db.Column(db.String(500))
    visited_entity_other = db.Column(db.String(255))
...
class Observation(TimestampMixin, db.Model):
    ...
    # نبقي التصنيف موجودًا لأن قاعدة البيانات الحالية تجعله إلزاميًا
    # لكن في الواجهة سنجعله مخفيًا بقيمة افتراضية "عام"
...
class ObservationAction(TimestampMixin, db.Model):
    id = db.Column(db.Integer, primary_key=True)
