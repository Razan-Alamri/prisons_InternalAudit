...
def home():
    ...
    if role in ['central_admin', 'central_operator', 'central_director']:
        ...
        return render_template(
            ...
            charts=charts,
            title='لوحة إدارة المراجعة الداخلية'
        )

    if role == 'region_manager':
        stats, mission_regions, charts = region_dashboard(current_user)
        return render_template(
            'dashboard/region_manager.html',
            stats=stats,
            mission_regions=mission_regions,
            charts=charts,
            title='لوحة المنطقة'
        )

    if role == 'executor':
        stats, assigned_reports, charts = executor_dashboard(current_user)
        return render_template(
            'dashboard/executor.html',
            stats=stats,
    ...
    if role == 'prison_director':
        ...
        return render_template(
            'dashboard/prison_director.html',
            stats=stats,
            mission_regions=mission_regions,
            charts=charts,
            title='لوحة مدير سجون المنطقة'
        )
